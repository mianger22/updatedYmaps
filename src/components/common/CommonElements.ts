...
export const CommonProperties = (object_data: PlaceInformationType, arrived: boolean, setArrived: any) => {
  ...
  return {
    ...
    balloonContentFooter: arrived ? `<div onclick="alert('Захвачена позиция ${object_data.name_of_place}!')">Захватить позицию</div>` : `<div onclick="window.changeStayStatus()">Я на месте</div>`,
